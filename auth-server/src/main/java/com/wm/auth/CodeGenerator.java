...
//		gc.setIdType(IdType.NONE);
        ...
        dsc.setUrl("jdbc:mysql://192.168.40.131:3306/auth-server?characterEncoding=UTF-8&zeroDateTimeBehavior" +
        ...
        pc.setParent(null);
        pc.setEntity("com.wm.auth.model.entity");
        pc.setMapper("com.wm.auth.mapper");
        pc.setXml("com.wm.auth.mapper.xml");
        String generateCaS = scanner("!!!是否覆盖当前的controller和service(是:1)");
        if ("1".equals(generateCaS)) {
            pc.setService("com.wm.auth.service");       //本项目没用，生成之后删掉
            pc.setServiceImpl("com.wm.auth.service.impl");   //本项目没用，生成之后删掉
            pc.setController("com.wm.auth.controller");
        } else {
